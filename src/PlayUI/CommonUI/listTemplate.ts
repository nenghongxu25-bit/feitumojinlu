...
export class listTemplate extends Laya.Script {
    ...
    private gridBox: {width:number;height:number;rotated:boolean} | null = null;

    public setGridBox(width:number,height:number,rotated=false):void {
        this.gridBox={width,height,rotated};
        this.resolveRuntimeBindings();
        const frame=this.templateSlot as any, name=this.nameText as any, count=this.countText as any;
        if(frame){frame.anchorX=frame.anchorY=0;frame.pos(0,0);frame.size(width,height);}
        if(name){name.pos(5,height-20);name.size(Math.max(1,width-10),18);name.fontSize=12;name.align='left';}
        if(count){count.pos(Math.max(0,width-45),3);count.size(40,18);count.fontSize=12;}
        this.applyIconSize(this.gimg as any);
    }
    public setPlacementColor(color:string):void {
        const frame=this.templateSlot as any;if(!frame)return;
        if(frame.background){frame.background.lineColor=color==='#ffffff'?'#394447':color;frame.repaint();}
        else frame.color=color;
    }
    public suppressGridClick():void {this.cancelLongPress();this.suppressNextClick=false;}
    ...
    private applyIconSize(icon: any): void {
        ...
        if(this.gridBox){
            const {width,height,rotated}=this.gridBox;
            icon.autoSize=false;icon.anchorX=icon.anchorY=0.5;icon.pos(width/2,(height-15)/2);
            const w=Math.max(20,width-12),h=Math.max(20,height-30);
            icon.size(rotated?h:w,rotated?w:h);icon.rotation=rotated?90:0;
            if('fitMode' in icon)icon.fitMode=2;
            return;
        }
    ...
    public setSelected(selected: boolean): void {
        ...
        const frame = this.templateSlot as any;
        if(frame?.background){frame.background.lineColor=selected?'#c4b984':'#394447';frame.background.fillColor=selected?'#303831':this.boundData?'#202b2c':'#141c20';frame.repaint();}
        else if (frame && "color" in frame) frame.color = selected ? "#d7c18a" : "#ffffff";
        const label = this.nameText as any;
        if (label && "color" in label) label.color = selected ? "#ead49d" : "#d4ddce";
    }
    ...
    private redrawDetailBackground(detail: any, width: number, height: number): void {
        ...
        if (graphics && typeof graphics.clear === "function" && typeof graphics.drawRect === "function") {
            graphics.clear();
            graphics.drawRect(0, 0, width, height, "#1d2a27", "#647565");
        }
