...
export class PlayerAnimationController {
    ...
    private playSpineTrack(animationName: string, loop: boolean, trackIndex: number): boolean {
        ...
        try {
            ...
            const index = Math.max(0, Math.floor(trackIndex || 0));
            // Spine2DRenderNode.play() treats its third argument as `force`.
            // The renderer underneath is the API that actually accepts a
            // track index; using it prevents upper-body actions from replacing
            // the locomotion track on mobile/runtime builds.
            if (render && typeof render.play === "function") {
                render.play(animationName, loop, index);
            } else {
                (this.spine as any).trackIndex = index;
                (this.spine as any).play(animationName, loop, true);
            }
            return true;
        } catch (error) {
    ...
    private finishActionSequence(token: number, onComplete?: () => void): void {
        ...
        if (this.spine) {
            ...
            if (this.controller.layeredSpineAnimationEnabled) {
                this.currentUpperAnimation = "";
                // Ending a shot must not restart the lower locomotion track.
                // For automatic fire this path runs once per bullet; forcing
                // the lower track here made the player's legs visibly twitch.
                this.syncLayeredLocomotion(fallbackAnimation, false);
            } else {
