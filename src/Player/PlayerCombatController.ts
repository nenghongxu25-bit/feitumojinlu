...
export class PlayerCombatController {
    ...
    private startAttack(options: PlayerAttackOptions = {}): void {
        ...
        const automaticInterval = isRanged ? Number(options.automaticIntervalMs) || 0 : 0;
        const lockDuration = automaticInterval > 0 ? Math.max(80, automaticInterval / attackSpeed) : Math.max(
        ...
        this.controller.animation.playActionAnimation(
            ...
            lockDuration,
            undefined,
            automaticInterval > 0 ? Math.max(attackSpeed, this.controller.attackAnimationDuration / lockDuration) : attackSpeed,
            () => {
        ...
        if (isRanged) {
            ...
            const hitDelay = Math.min(Math.max(0, this.controller.rangedAttackHitDelay || 0), Math.max(0, finishDelay - 1));
