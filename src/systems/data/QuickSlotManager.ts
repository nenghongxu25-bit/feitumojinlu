...
export class QuickSlotManager {
    ...
    public assignActiveSlot(quickSlotIndex: number, activeSlotIndex: number): boolean {
        ...
        const grid=this.data.inventory.getGridStorage();
        let next=InventoryGrid.clone(grid.items);next[slotIndex]=null;
        const previousQuickItem = this.items[index];
        if (previousQuickItem) {
            const item=grid.normalizer(previousQuickItem);
            next=InventoryGrid.add(next,[item],grid.spec,grid.stack,slotIndex) || InventoryGrid.add(next,[item],grid.spec,grid.stack);
            if(!next)return false;
        }
        this.items[index] = {...sourceItem};
        this.data.inventory.commitGrid(next);
    ...
    public moveToActiveSlot(sourceQuickSlotIndex: number, targetActiveSlotIndex: number): boolean {
        ...
        const grid=this.data.inventory.getGridStorage();
        const previousActiveItem = grid.items[targetIndex];
        if (previousActiveItem && !this.canAssignItem(previousActiveItem.itemId || "")) {
            return false;
        }
        const next=InventoryGrid.clone(grid.items);next[targetIndex]=null;
        const incoming=grid.normalizer(sourceItem);
        if(!InventoryGrid.fits(next,incoming,targetIndex,grid.spec))return false;
        next[targetIndex]={...incoming,gridVersion:2};
    ...
    public clearMissingItems(): void {
        ...
        for (let i = 0; i < this.items.length; i++) {
            ...
            // Disabled saved items stay in their slots; activation is guarded separately.
            if (item && (!item.itemId || item.count <= 0 ||
                (!this.canAssignItem(item.itemId) && this.data.resolveItemMeta(item.itemId)?.enabled !== false))) {
                this.items[i] = null;
                changed = true;
            }
        }
    ...
    public load(): void {
        ...
        for (let i = 0; i < this.items.length; i++) {
            ...
            this.items[i] = itemId && count > 0 && (this.canAssignItem(itemId) || this.data.resolveItemMeta(itemId)?.enabled === false)
                ? {
                      itemId,
