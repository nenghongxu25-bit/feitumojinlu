...
export class ForestWeatherMask {
    ...
    /** Read the painted ground bounds on each scene load; no fixed map-size maintenance. */
    static fitToGround(ground: Laya.Sprite | null, overlay: Laya.Sprite): boolean {
        const layer = ground?.getComponent(Laya.TileMapLayer);
        const rect = layer?.rect;
        if (!rect || !overlay.parent || rect.z <= rect.x || rect.w <= rect.y) return false;
        const parent = overlay.parent as Laya.Sprite;
        const min = ground.localToGlobal(new Laya.Point(rect.x, rect.y), true);
        const max = ground.localToGlobal(new Laya.Point(rect.z, rect.w), true);
        parent.globalToLocal(min, false); parent.globalToLocal(max, false);
        overlay.pos(min.x, min.y);
        overlay.size(max.x - min.x, max.y - min.y);
        return true;
    }
    ...
    static readonly glsl = `
        vec2 forestSurface(vec2 point) {
            if (u_terrainGrid.x<0.5) return vec2(1.0,0.0);
            vec2 grid=point/u_terrainGrid.zw;
            vec2 cell=floor(grid);
            if (cell.x<0.0 || cell.y<0.0 || cell.x>=u_terrainGrid.x || cell.y>=u_terrainGrid.y) return vec2(0.0);
