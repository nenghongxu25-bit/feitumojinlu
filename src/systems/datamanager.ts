...
import { InventoryGrid } from "./data/InventoryGrid";
import { cleanLegacyInventorySaves } from "./data/LegacyInventoryCleanup";
...
export class DataManager {
    ...
    public async loadAll(): Promise<void> {
        ...
        try {
            const [materials, foods, weapons, misc, medicines, harvest, equipment] = await Promise.all([
                ...
                this.loadJson<any>("config/equipment-design-v1.json", "assets/config/equipment-design-v1.json"),
            ]);
            ...
            this.registerContainerEquipment(equipment);
    ...
    private cleanRetiredInventory(): void {
        // Retain weapons even if their availability is toggled later; allow future enabled loot.
        const keep = (id: string) => {
            const meta = this.resolveItemMeta(id);
            return !!meta && (meta.category === 'weapons' || meta.enabled !== false);
        };
        cleanLegacyInventorySaves(this.save, keep);
        this.inventory.removeRetiredItems(keep);
    }
    ...
    public grantHarvestDrops(harvestId: string, fallback: HarvestDropConfig[] = []): HarvestResultItem[] | null {
        const results = this.harvest.rollHarvestDrops(harvestId, fallback);
        if(!this.inventory.canAddItems(results))return null;
    ...
    public grantItemsToActive(items: Array<{ itemId: string; name?: string; count: number; icon?: string }>): void {
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            if (!this.isItemEnabled(item?.itemId)) continue;
    ...
    public canGrantItemsToActive(items: Array<{ itemId: string; name?: string; count: number; icon?: string }>): boolean {
        if (items.some((item) => !this.isItemEnabled(item?.itemId))) return false;
    ...
    public canUseItem(itemId: string): boolean {
        if (!this.isItemEnabled(itemId)) return false;
    ...
    public applyItemUseStats(itemId: string): void {
        if (!this.isItemEnabled(itemId)) return;
    ...
    public getSignInRewards(): SignInRewardView[] {
        return this.signIn.getRewardViews((reward) => this.resolveSignInRewardView(reward))
            .filter((reward) => this.isItemEnabled(reward.itemId));
    }
    ...
    public claimSignInReward(day: number): boolean {
        if (!this.getSignInRewards().some((reward) => reward.day === day)) return false;
    ...
    public isItemEnabled(itemId: string): boolean {
        return this.items.isItemEnabled(itemId);
    }
    ...
    private normalizeInventoryItem(item: InventoryViewItem): InventoryViewItem {
        ...
        const meta = this.resolveItemMeta(itemId);
        const state=meta?.storageStates?(item.state==='folded'?'folded':'expanded'):undefined;
        const shape=state?meta.storageStates[state]:null;
        return {
            ...item,
            ...(state?{state,contents:item.contents||[]}:{}),
            gridWidth: shape?.gridWidth || meta?.gridWidth || item.gridWidth || 1,
            gridHeight: shape?.gridHeight || meta?.gridHeight || item.gridHeight || 1,
            itemId,
            name: this.resolveDisplayName(itemId, item.name),
            count: item.count,
            icon: shape?.icon || item.icon || meta?.icon || this.resolveFallbackIcon(itemId),
        };
    }

    public registerContainerEquipment(catalog:any):void {
        for(const category of ['backpack','rig'])this.items.registerItemTable({category,items:(catalog.items||[]).filter((i:any)=>i.category===category&&i.storageStates&&i.container).map((i:any)=>({...i,enabled:true,displayName:i.name,nameZh:i.name,stackMax:1}))});
    }

    public planContainerToggle(items:InventorySlotItem[],index:number,spec:import('./data/InventoryGrid').GridSpec):{items:InventorySlotItem[]|null;error:string|null} {
        const item=items[index],meta=item?this.resolveItemMeta(item.itemId):null;
        if(!item||!meta?.storageStates)return {items:null,error:'该物品不能卷起'};
        const state=item.state==='folded'?'expanded':'folded';
        if(state==='folded'&&item.contents?.some(Boolean))return {items:null,error:'请先清空内部物资，再卷起'};
        const changed=this.normalizeInventoryItem({...item,state});
        if(!InventoryGrid.fits(items,changed,index,spec,index))return {items:null,error:'展开空间不足，请先移动到有足够空格的位置'};
        const next=InventoryGrid.clone(items);next[index]=changed;
        return {items:next,error:null};
    }

    public toggleContainerState(bucket:InventoryBucket,index:number):string|null {
        const grid=bucket==='active'?this.inventory.getGridStorage():this.warehouse.getGridStorage();
        const plan=this.planContainerToggle(grid.items,index,grid.spec);
        if(plan.items)this.commitInventoryGrid(bucket,plan.items);
        return plan.error;
    }
    ...
    public equipItemFromActive(slot:EquipmentSlotType,itemId:string):boolean {
        const index=this.inventory.getGridStorage().items.findIndex(i=>i?.itemId===itemId);
        return this.equipGridItem('active',index,slot);
    }

    public equipGridItem(bucket:InventoryBucket,index:number,slot:EquipmentSlotType):boolean {
        const grid=bucket==='active'?this.inventory.getGridStorage():this.warehouse.getGridStorage(),item=grid.items[index];
        ...
        this.equippedItems[slot]={...item,itemId,count:1};
        this.commitInventoryGrid(bucket,next);this.saveEquipment();this.clearQuickSlotsForMissingItems();return true;
    }

    private commitInventoryGrid(bucket:InventoryBucket,items:InventorySlotItem[]):void {
        if(bucket==='active')this.inventory.commitGrid(items,false);else this.warehouse.commitGrid(items);
        this.inventory.refreshBagViews();this.syncWarehouseViews();
    }

    public organizeInventory(bucket:InventoryBucket):boolean {
        const grid=bucket==='active'?this.inventory.getGridStorage():this.warehouse.getGridStorage();
        if(!grid.organize(item=>this.resolveInventorySortPriority(item)))return false;
        this.commitInventoryGrid(bucket,grid.items);return true;
    }

    public splitInventoryStack(bucket:InventoryBucket,index:number,count:number):boolean {
        const grid=bucket==='active'?this.inventory.getGridStorage():this.warehouse.getGridStorage(),item=grid.items[index];
        if(!item||!Number.isInteger(count)||count<1||count>=item.count)return false;
        const next=InventoryGrid.clone(grid.items),piece={...item,count};
        const target=InventoryGrid.first(next,piece,grid.spec);if(target<0)return false;
        next[index].count-=count;next[target]=piece;this.commitInventoryGrid(bucket,next);return true;
    }

    public discardInventorySlot(bucket:InventoryBucket,index:number):boolean {
        const grid=bucket==='active'?this.inventory.getGridStorage():this.warehouse.getGridStorage();
        if(!grid.items[index])return false;
        const next=InventoryGrid.clone(grid.items);next[index]=null;
        this.commitInventoryGrid(bucket,next);this.clearQuickSlotsForMissingItems();return true;
    }
    ...
    public getEquipmentDefenseBonus(): number {
        ...
        for (let i = 0; i < slots.length; i++) {
            ...
            const defense = item && this.isItemEnabled(item.itemId) ? this.resolveItemMeta(item.itemId)?.defense : 0;
    ...
    public transferLooseItemToActive(item: { itemId?: string; name?: string; count: number; icon?: string; rotated?:boolean } | null, targetSlotIndex?: number, preview=false): boolean {
        if (!this.isItemEnabled(item?.itemId)) return false;
        if (!item || !item.itemId || !Number.isFinite(item.count) || item.count <= 0) {
            return false;
        }
        ...
        const next=this.inventory.getGridStorage().planAdd([{...resolvedItem,rotated:!!item.rotated}],targetSlotIndex);
        if(!next)return false;
        if(preview)return true;
        this.inventory.commitGrid(next);
        this.refreshQuickSlotViews();
        return true;
    }
