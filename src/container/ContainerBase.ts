...
import { ContainerOutline } from "../systems/ContainerOutline";
import { BrickWallTileLayer } from "../systems/BrickWallTileLayer";
import { DepthSortable } from "../systems/DepthSortable";
...
export abstract class ContainerBase extends Laya.Script {
    ...
    private outline: ContainerOutline | null = null;

    public isVisibleToPlayer(): boolean {
        const node = this.owner as Laya.Sprite;
        if (!this.enabled || !node.activeInHierarchy || !node.visible) return false;
        const groundY = node.getComponent(DepthSortable)?.groundY ?? node.height;
        const foot = node.y + (groundY - node.pivotY) * node.scaleY;
        return BrickWallTileLayer.targetVisible(node, foot) !== false;
    }

    onLateUpdate(): void {
        if (!this.outline) this.outline = new ContainerOutline(this.owner as Laya.Sprite);
        const player = PlayerController.activeInstance;
        this.outline.update(this.isVisibleToPlayer(), ContainerBase.getFocusedTarget() === this
            && this.isAvailable() && !!player && !player.animation.isBusy());
    }
    ...
    /** Optional visual frame sequence played when this container is opened. */
    ...
    public static getFocusedTarget(): ContainerBase | null {
        const target = ContainerBase.focusedTarget;
        return target?.isVisibleToPlayer() ? target : null;
    }
    ...
    public open(player: PlayerController | null): boolean {
        if (!this.isVisibleToPlayer()) return false;
        if (!this.isAvailable()) {
    ...
    private rollContents(): InventoryViewItem[] {
        ...
        for (let i = 0; i < contents.length; i++) {
            ...
            if (!dataManager.isItemEnabled(item.itemId)) continue;
    ...
    private playOpenVisualAnimation(): void {
        const frames = this.getOpenVisualFrames();
        if (frames.length === 0) {
            return;
        }
        ...
        for (let i = 0; i < frames.length; i++) {
            Laya.timer.once(i * 125, this, () => {
                if (!owner.destroyed && image) {
                    image.src = frames[i];
                }
            });
        }
