...
export class DynamicCutoutProbe extends Laya.Script {
    ...
    @property({ type: Boolean, caption: "启用环境闪电" })
    public lightningEnabled = true;
    @property({ type: Number, caption: "闪电强度" })
    public lightningIntensity = 0.88;
    @property({ type: Number, caption: "闪电持续秒数" })
    public lightningDuration = 0.5;
    @property({ type: Number, caption: "闪电最短间隔秒数" })
    public lightningMinInterval = 2;
    @property({ type: Number, caption: "闪电最长间隔秒数" })
    public lightningMaxInterval = 2;
    private readonly lightning = new LightningFlash();

    private diagnostics: NightLightingDiagnostics | null = null;

    onDisable(): void {
        this.lightning.reset(this.owner as Laya.Sprite);
    ...
    private fireRoot: Laya.Node | null = null;
    ...
    onLateUpdate(): void {
        ...
        if (this.backgroundNode) {
            const x = -this.backgroundNode.width * 0.5 + RangedAimState.cameraOffsetX;
            const y = -this.backgroundNode.height * 0.5 + RangedAimState.cameraOffsetY;
            if (this.backgroundNode.x !== x || this.backgroundNode.y !== y) {
                this.backgroundNode.pos(x, y);
                owner.reCache();
            }
        }
        const cutout = this.cutoutNode;
        ...
        if (!this.roomController || this.roomController.destroyed) {
            const world = owner.parent;
            const rooms = world?.getChildByName("roomnight") ||
                world?.getChildByName("LogicLayer")?.getChildByName("roomnight");
            this.roomController = rooms?.getComponent(RoomNightController) || null;
        }
        if (this.roomController?.enabled) this.roomController.updateAfterFlashlight();
        this.lightning.update(owner, dt, this.lightningEnabled, this.lightningIntensity,
            this.lightningDuration, this.lightningMinInterval, this.lightningMaxInterval);
        if (!this.fireRoot || this.fireRoot.destroyed) this.fireRoot = owner.parent.getChildByName("fire") ||
            owner.parent.getChildByName("EffectLayer")?.getChildByName("fire");
        if (this.fireRoot) {
            for (let i = 0; i < this.fireRoot.numChildren; i++) {
                const light = this.fireRoot.getChildAt(i).getComponent(FireVisualLight);
                if (light?.enabled) light.updateAfterNight(owner);
            }
        }
        RoomDarkness.updateAfterNight(owner);
    }
