...
import { RoomFlashlightCutout } from "./RoomFlashlightCutout";
...
export class RoomNightZone extends Laya.Script {
    ...
    @property({ type: Laya.Sprite, caption: "手电扣除节点" })
    public flashlightCutout: Laya.Sprite | null = null;

    private readonly flashlight = new RoomFlashlightCutout();

    public updateFlashlight(source: Laya.Sprite | null, world: Laya.Sprite): void {
        this.flashlight.update(this.owner as Laya.Sprite, this.flashlightCutout, source, world);
    }

    @property({ type: Number, caption: "室外遮挡强度" })
    public outsideAlpha: number = 1;

    @property({ type: Number, caption: "室内遮挡强度" })
    public insideAlpha: number = 0.25;
    ...
    public updateRoom(reveal: boolean, isNight: boolean, targetNight: Laya.Sprite,
        ...
        const outside = Math.max(0, Math.min(1, this.outsideAlpha));
        const inside = Math.max(0, Math.min(1, this.insideAlpha));
        const desiredAlpha = outside + (inside - outside) * revealAmount;
    ...
    public resetRoom(): void {
        ...
        if (this.flashlightCutout && !this.flashlightCutout.destroyed) {
            this.flashlightCutout.visible = false;
            room.reCache();
        }
