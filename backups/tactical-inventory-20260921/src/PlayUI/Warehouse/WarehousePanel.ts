...
export class WarehousePanel extends Laya.Script {
    ...
    private bindWarehouseList(): void {
        ...
        this.warehouseGlist.slotOffset = this.currentWarehousePage * WarehouseManager.PAGE_SIZE;
    ...
    private handleSlotClick(sourceBucket: InventoryBucket, item: ListTemplateData | null, listKey: string, slotIndex: number): void {
        ...
        if (!itemId) {
            if (this.selectedSlot.bucket !== targetBucket && DataManager.getInstance().moveGridItem(this.selectedSlot.bucket, this.selectedSlot.slotIndex, targetBucket, targetSlotIndex)) {
                this.clearSelection();
                this.refresh();
                return;
            }
        ...
        if (DataManager.getInstance().moveGridItem(this.selectedSlot.bucket, this.selectedSlot.slotIndex, targetBucket, targetSlotIndex)) {
            this.clearSelection();
            this.refresh();
            return;
        }
    ...
    private getWarehousePageSelectedSlotIndex(): number {
        ...
        const pageEnd = pageStart + this.getVisiblePageSize();
    ...
    private toWarehousePageData(items: InventorySlotItem[]): Array<ListTemplateData | null> {
        ...
        const pageItems = Array.isArray(items) ? items.slice(startIndex, startIndex + this.getVisiblePageSize()) : [];
    ...
    private getVisiblePageSize(): number {
