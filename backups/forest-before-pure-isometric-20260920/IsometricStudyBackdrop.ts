...
export class IsometricStudyBackdrop extends Laya.Script {
    ...
    @property(Boolean) orthographic = true;
    @property(Number) orthographicSize = 9;
    ...
    async onStart(): Promise<void> {
        ...
        camera.orthographic=this.orthographic;camera.fieldOfView=this.fieldOfView;
        camera.orthographicVerticalSize=this.orthographicSize;
        ...
        // Retreat along the same viewing axis to keep the orthographic near plane above the ground.
        if(this.orthographic)camera.transform.position=new Laya.Vector3(0,this.lookHeight+(this.cameraHeight-this.lookHeight)*2,this.cameraDistance*2);
        ...
        // A finite rear edge exposes the sky in an orthographic view.
        if(this.orthographic){base.transform.position=new Laya.Vector3(0,-.025,46.5);base.transform.localScale=new Laya.Vector3(200,1,107);}
        ...
        console.log("[ForestPerspective] ready: "+(this.orthographic?"orthographic":"perspective")+" camera, ground tiles="+count+", upright props="+props.numChildren);
    }
